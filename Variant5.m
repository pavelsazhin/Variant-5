...
if  length(x)==0 | length(y)==0 | isnan(n) == 1
    errordlg('Âñå ïîëÿ ââîäà äîëæíû áûòü çàïîëíåíû', 'Îøèáêà');
else if n < 0
    errordlg('Ñòåïåíü ïîëèíîìà äîëæíà áûòü áîëüøå íóëÿ', 'Îøèáêà');
    elseif length(x) ~= length(y)
        errordlg('Êîëè÷åñòâî çíà÷åíèé X è Y äîëæíî áûòü îäèíàêîâûì', 'Îøèáêà');
    else
    p = polyfit(x,y,n);
...
helpdlg({'Программа выполняет приближение по методу наименьших квадратов. На входе вводятся координаты точек (X и Y). На выходе строиться график, а в таблицу заносяться коэффициенты полинома.','','','Для корректной работы программы необходимо, чтобы: ','1) Все поля ввода были заполнены.', '2) Количество значений X и Y совпадало.', '3) Степень полинома была больше нуля.'}, 'Помощь');
